const parsers = {
  ...
  matrimonio: (texto) => {
    ...
    const parroquiaMatch = textoFlat.match(/[PD]arroquia\s+([^,\n]+)/i);
    if (parroquiaMatch) datos.parroquia = parroquiaMatch[1].trim();
    ...
    const contrayenteMatch = textoFlat.match(/Contrayente[:\s]+(.+?)\s*,?\s*nacido/i);
    if (contrayenteMatch) datos.nombre_contrayente = contrayenteMatch[1].trim();

    const contrayentaMatch = textoFlat.match(/Contrayente[:\s]+(.+?)\s*,?\s*nacida/i);
    if (contrayentaMatch) datos.nombre_contrayenta = contrayentaMatch[1].trim();

    const lugarMatch = textoFlat.match(/tuvo lugar en\s+(.+?)(?=\s{2,}|Actuaron|[,.])/i);
    ...
    const testigosMatch = textoFlat.match(/testigos?:\s*(.+?)\s+y\s+(.+?)(?=\s*,|\s*mayores|\s*\.)/i);
    if (testigosMatch) {
